...
export async function sendEmail({ to, subject, html }: SendEmailInput) {
  ...
  const from = process.env.EMAIL_FROM ?? "211Motors <onboarding@resend.dev>";
...
export function autoresponderEmail(params: { fullName: string; reference?: string }) {
  return {
    subject: "We've received your enquiry — 211Motors",
    html: `
      <p>Hi ${params.fullName},</p>
      <p>Thank you for contacting 211Motors, the sole authorized distributor of Soueast and 212 vehicles in South Sudan and Sudan.
